...
class PQDictionary():
    def __init__(self, data, pqParams, depth, centers = None):
        ...
        self.distanceEDMattrix=None
        self.productQuantisers = []
        params = pqParams[depth]
        self.params=params

      
        if params.distanceMetric is Metric.ED:
            if params.residuals:
                self.avg = np.mean(data)
                data = data - self.avg
            self.kmeans = KMeans(n_clusters=params.dictionarySize, random_state=0).fit(data)
            self.codeBook = self.kmeans.cluster_centers_
            if params.withIndex:
                self.index=np.array([]*k, dtype=np.int)
                pass
        ...
        if params.distanceMetric is Metric.DTW:
            if params.residuals:
                ...
                pass        
            self.kmeans = TimeSeriesKMeans(n_clusters=params.dictionarySize,random_state=0,metric="dtw",).fit(data)
            self.codeBook = self.kmeans.cluster_centers_
            if params.withIndex or self.recursiveLayer:
                predictions = self.kmeans.predict(data)
                self.index=[]
                for i in range(0,params.dictionarySize):
    ...
    def __init__(self,data, pqParams, depth=0):
        ...
        if pqParams[depth].quantizerType==QuantizerType.PQNeedlemanWunsch or pqParams[depth].quantizerType==QuantizerType.VQNeedlemanWunsch:
            ...
            for i in range(1, self.nrDictionaries):
                codeBook = np.concatenate((codeBook,self.dictionaries[i].codeBook), axis=0)
            self.substitution = SparseSubstitutionMatrix(codeBook,pqParams[depth].windowSize, pqParams[depth].psi)
