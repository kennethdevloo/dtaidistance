...
cimport numpy as np
import math
from .dtw_c import distance_matrix, distance
...
cdef class PQDictionary():
    ...
    cpdef retrieveCodes(self, np.ndarray[np.double_t, ndim=2] data):
        print(distance_matrix(np.concatenate((data, np.reshape(self.kmeans.cluster_centers_, (self.kmeans.cluster_centers_.shape[0],self.kmeans.cluster_centers_.shape[1])))),block=((0,data.shape[0]), (data.shape[0], data.shape[0]+self.kmeans.cluster_centers_.shape[0])),**self.params.distParams ).shape)
        ...
        cdef np.ndarray[np.int32_t, ndim=1] distMatrix2=np.argmin(distMatrix, axis=1)[0:len(data)]
    ...
    cdef double retrieveRecApprDTWDistance(self, int code, np.ndarray[np.int32_t, ndim=1] data1, np.ndarray[np.int32_t, ndim=1] data2):
...
cdef class ProductQuantizer():
    ...
    cdef np.ndarray[np.int32_t, ndim = 2] retrieveCodedData(self, np.ndarray[np.double_t,ndim=2] data):
        cdef np.ndarray[np.int32_t, ndim=2] codedData
        cdef int i
        
        if self.params.subsetType is SubsetSelectionType.NO_OVERLAP:
            codedData = np.zeros((data.shape[2],self.nrDictionaries), dtype=np.int32)
        if self.params.subsetType is SubsetSelectionType.DOUBLE_OVERLAP:
            codedData = np.zeros((data.shape[0],self.nrDictionaries*2-1), dtype=np.int32)
    ...
    cdef calculateApprDTWDistanceForCodes(self, np.ndarray[np.int32_t, ndim=1] code1, np.ndarray[np.int32_t, ndim=1] code2, np.ndarray[np.double_t, ndim=1] data1, np.ndarray[np.double_t, ndim=1] data2):
    ...
    cdef constructApproximateDTWDistanceMatrixExecute(self, np.ndarray[np.double_t, ndim = 2] data):
        ...
        cdef np.ndarray[np.int32_t, ndim=2] codedData = self.retrieveCodedData(data)
    ...
    cdef double calculateApprDTWDistanceForData(self, np.ndarray[np.double_t, ndim=1] data1,np.ndarray[np.double_t, ndim=1] data2):
        cdef np.ndarray[np.int32_t, ndim=2] codedData = self.retrieveCodedData(np.array([data1, data2]))
...
class PyProductQuantizer():
    ...
    def constructApproximateDTWDistanceMatrix(self, data):
        return self.pq.constructApproximateDTWDistanceMatrix(data)
